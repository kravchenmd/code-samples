...
def find_text(notes, args: list):
    ...
    for note in notes:
        if str(note).find(text) != -1:
            ...
            flag += 1
    if flag == 0:
        result = f'There are no "{text}" symbols in the notebook'
    return result


# TODO 1 (Lara): change signature to `def edit_note(notebook, note_number, *args)`
...
def edit_note(notes, *args):
    ...
    if note_number < len(notes):
        ...
        result = f'There is no note with number {note_number + 1} in the notebook'
    return result
...
def exit_notes(notes, *args):
    ...
    return f'Your notebook saved in file {path}. Good bye!'
