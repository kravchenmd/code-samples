...
async def get_all_notes(db: Session, user: User, skip: int, limit: int):
    all_notes = db.query(Note).filter(Note.user == user.id).offset(skip).limit(limit).all()
    return all_notes
...
async def create_note(db: Session, note: NoteModel, user: User):
...
async def update_note(db: Session, note_id: int, new_note: NoteUpdate, user: User):
    note = db.query(Note).filter(and_(Note.id == note_id, Note.user == user.id)).first()
    if note:
...
async def done_note(db: Session, note_id: int, new_note: NoteDone, user: User):
    note = db.query(Note).filter(and_(Note.id == note_id, Note.user == user.id)).first()
    if note:
        note.done = new_note.done
        db.commit()
